...
%ENGINE NOISE MODEL:
n = 3000;%engine rotational speed in RPM
...
n_p = n;%propeller rotational speed (rpm)
r = alt_m(alt);%distance from propeller (m)
c = a_si(alt);%speed of sound(m/s)
N = 1;%number of propellers
M_t = (pi*D*n_p)/(60*c);%tip mach number
V = 0;%forward flight airspeed,m/s
...
L_PP_1 = 10.*log10((10.^(0.1.*SPL_prop)) + (10.^(0.1.*SPL_engine)))
